 ...
 */
public class PlayBackActivity extends BaseActivity implements View.OnTouchListener {
    ...
    public class MyCallBack extends ICallBack {
        ...
        public void onPlayBackMsgDetails(PlayBackMsgResultBean msgResultBean) {
            ...
            if (msgResultBean.getReturnCode() == 0) {
                ...
                if (msgListResult != null && msgListResult.size() > 0) {
                    upSendAt = msgListResult.get(0).getSendAt();
                    items.addAll(msgListResult);
                    // 消息记录:1481876276240
                    AppLog.i("TAG", "消息记录:" + upSendAt + "" + "发送人:" + msgListResult.get(0).getFromNick() + "   聊天内容:" + msgListResult.get(0).getContent());
                }
        ...
        public void onLiveUserInfo(LiveUserInfosDataResp liveUserInfosDataResp) {
            ...
            if (liveUserInfosDataResp.getReturnCode() == 0) {
                ...
                if (isUnDestroy) {
                    CustomNewUserInforDialog dialog = new CustomNewUserInforDialog(PlayBackActivity.this, null, String.valueOf(id), intentId, 0, false, null, null);
                    dialog.show();
                }
            }
        }
    ...
    boolean traveralComplete=true;
    private void getMsgInfo(long msgAt) {
        try {
            if (upSendAt != 0) {
                if (msgAt > upSendAt) {
                    if(traveralComplete){
                        traveralComplete=false;
                    }
                    if (thisPoll != null) {
                    ...
                    thisPoll = items.poll();
                    if (thisPoll != null) {
                        upSendAt = thisPoll.getSendAt();
                        getMsgInfo(msgAt);
                    } else {
                    }
                }else{
                    traveralComplete=true;
                }
            }
        }catch (Exception e){
            e.printStackTrace();
    ...
    private void showToastTips(final String tips) {
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                Toast.makeText(PlayBackActivity.this, tips, Toast.LENGTH_SHORT).show();
            }
        });
    }
    ...
    protected Handler mHandler = new Handler(Looper.getMainLooper()) {
        ...
        public void handleMessage(Message msg) {
            ...
            if (!Utils.isNetworkAvailable(PlayBackActivity.this)) {
                sendReconnectMessage();
                return;
            }
