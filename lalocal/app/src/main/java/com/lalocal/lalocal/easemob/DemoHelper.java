...
import com.lalocal.lalocal.easemob.domain.EmojiconExampleGroupData;
import com.lalocal.lalocal.easemob.ui.ChatActivity;
import com.lalocal.lalocal.easemob.utils.ListenerManager;
import com.lalocal.lalocal.easemob.utils.PreferenceManager;
        ...
        easeUI.setEaseUserInfoProvider(new EaseUI.EaseUserInfoProvider() {
            ...
            public void setNickAndAvatar(Context context, EMMessage message, ImageView userAvatarView, TextView usernickView) {
                ...
                if (message.direct == EMMessage.Direct.SEND) {
                    EaseUserUtils.setUserAvatar(context, EMChatManager.getInstance().getCurrentUser(), userAvatarView,1);
                    //发送方不显示nick
                    //            UserUtils.setUserNick(EMChatManager.getInstance().getCurrentUser(), usernickView);
                } else {
                    if (jsonAgent == null) {
                        userAvatarView.setImageResource(com.easemob.easeui.R.drawable.service_header_default);
...
//                            usernickView.setText(message.getFrom());
                        ...
                        if (!TextUtils.isEmpty(strUrl)) {
                            ...
                            //正常的string路径
                            Glide.with(context).load(strUrl).diskCacheStrategy(DiskCacheStrategy.ALL).placeholder(com.easemob.easeui.R.drawable.service_header_default).into(userAvatarView);
                        } else {
                            Glide.with(context).load(com.easemob.easeui.R.drawable.service_header_default).into(userAvatarView);
                        }
                    }
                }
            }
