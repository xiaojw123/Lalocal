...
public class PlayBackNewActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        intentId = getIntent().getStringExtra("id");  //1659
    ...
    private void parseIntent(PlayBackResultBean liveRowsBean){
        ...
        long startAtLong = stringToDate(startAt, "yyyy-MM-dd HH:mm:ss");
        ...
        switch (view.getId()) {
            ...
            case R.id.playback_top_share:
                if (shareVO != null&&isUnDestroy) {
                    ...
                    Toast.makeText(PlayBackNewActivity.this, "此视频暂不可分享!!", Toast.LENGTH_SHORT).show();
                }
    ...
    private VideoPlayCallbackImpl mVideoPlayCallback = new VideoPlayCallbackImpl() {
        ...
        public void onPlayFinish() {
            ...
            if (videoList.size() == 1) {
                Toast.makeText(PlayBackNewActivity.this, "视频播放完成!", Toast.LENGTH_SHORT).show();
                return;
            } else if (videoList.size() == position + 1) {
                Toast.makeText(PlayBackNewActivity.this, "视频全部播放完毕!", Toast.LENGTH_SHORT).show();
                return;
            } else {
                Toast.makeText(PlayBackNewActivity.this, "视频播放完毕，正加载下一段视频...", Toast.LENGTH_SHORT).show();
        ...
        public void onClickBefore(ImageView view) {
            ...
            if (position == 0) {
                Toast.makeText(PlayBackNewActivity.this, "已经是第一段视频了!", Toast.LENGTH_SHORT).show();
                return;
            }

            if (videoList.size() == 1) {
                Toast.makeText(PlayBackNewActivity.this, "没有上一段视频!", Toast.LENGTH_SHORT).show();
                return;
            }
            --position;
            if (videoList.size() == position || position < 0) {
                ...
                Toast.makeText(PlayBackNewActivity.this, "正在加载上一段视频....", Toast.LENGTH_SHORT).show();
            }
            positionChangeListener(position);
        ...
        public void onClickNext(ImageView view) {
            ...
            if (videoList.size() == 1) {
                Toast.makeText(PlayBackNewActivity.this, "没有下一段视频!", Toast.LENGTH_SHORT).show();
                return;
            }
            ++position;
            ...
            if (videoList.size() == position) {
                ...
                Toast.makeText(PlayBackNewActivity.this, "正在加载下一段视频....", Toast.LENGTH_SHORT).show();
            }
            positionChangeListener(position);
            Uri uri = Uri.parse(videoList.get(position).getUrl());
            if ((videoPlayer.getVisibility()) == 0) {
    ...
    public class MyCallBack extends ICallBack {
        ...
        public void onPlayBackMsgDetails(PlayBackMsgResultBean msgResultBean) {
            ...
            if(msgResultBean.getReturnCode()==0){
                List<PlayBackMsgResultBean.ResultBean> result = msgResultBean.getResult();
                long sendAt = result.get(0).getSendAt();
                if(result!=null&&result.size()>0){
                    items.addAll(result);
                }
                // 消息记录:1481876276240
                AppLog.i("TAG","消息记录:"+sendAt+"");
            }
        }
